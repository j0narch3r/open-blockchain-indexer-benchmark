...
```sql
select count(*) from `GasSpent` limit 4
```

## Performance Results

In the benchmark test, this Sentio processor completed indexing of all LBTC transfer events in **35.6 minutes**. 

| Avg_process_time | max_process_time | min_process_time |
|------------------|------------------|------------------|
| 47.9ms           | 38,636ms         | 1ms              |
